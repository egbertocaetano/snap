...
void processaATRIBUICAO(atributos * dolar, atributos * dolar1, atributos * dolar2, atributos * dolar3)
{
	...
	{	
		if((*tab1)[dolar1->label].tipo == "string")
		{
			...
			(*tab)[dolar1->label].tmp = dolar->tmp;
			(*tab)[dolar1->label].tmp = dolar->label;
			(*tab)[dolar1->label].tamanhoString = dolar->tamanhoString;
			(*tab)[dolar1->label].tipo = dolar->tipo;
		...
		else
		{
			...
			dolar->tmp = (*tab1)[dolar1->label].tmp;
			dolar->label = (*tab1)[dolar1->label].label;
			dolar->tamanhoString = tamanho;
			dolar-> tipo = (*tab1)[dolar1->label].tipo;
	...
	else
	{
		if((*tab1)[dolar1->label].tipo == "string")
		{
			...
			(*tab)[dolar1->label].tmp = dolar->tmp;
			(*tab)[dolar1->label].label = dolar->label;
			(*tab)[dolar1->label].tipo = dolar->tipo;
			(*tab)[dolar1->label].tamanhoString = dolar->tamanhoString;
...
void operacaoAritmetica(atributos * dolar, atributos * dolar1, atributos * dolar2, atributos * dolar3)
{
	...
	if(tab1 != NULL)
	{
		(*tab1)[dolar1->label].tmp =  dolar1->tmp;
		(*tab1)[dolar1->label].label = dolar1->label;
		(*tab1)[dolar1->label].tipo = dolar1->tipo;
	}

	if(tab2 != NULL)
	{
		(*tab2)[dolar3->label].tmp =  dolar3->tmp;
		(*tab2)[dolar3->label].label = dolar3->label;
		(*tab2)[dolar3->label].tipo = dolar3->tipo;
	}
}
...
void operacaoRelacional(atributos * dolar, atributos * dolar1, atributos * dolar2, atributos * dolar3)
{
	...
	if(dolar1->tipo != dolar3->tipo)
	{
		...
		dolar->tipo = tipo;	
		dolar->traducao = dolar1->traducao + dolar3->traducao + "\t" + 
						  dolar->tmp + " = " + dolar1->tmp + " " + 
						  dolar2->operador + " "+ dolar3->tmp + ";\n";
